...
def test_tier3_fallback(mock_settings):
    ...
    ai_matches, exceptions = investigate_exceptions(unmatched, [])
    assert len(ai_matches) == 0
    assert len(exceptions) == 1
    assert exceptions[0].diagnosis_category == "manual_review_required"
    assert "skipped" in exceptions[0].explanation.lower()
